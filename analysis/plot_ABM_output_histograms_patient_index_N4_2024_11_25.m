...
varnames = {'VDbl', 'ACHp', 'ACHb', 'pShared', 'index_cfg',...
            'p1_AR', 'p5_AR', 'q90_AR', 'mean_AR',...
            'p1_ARpatient','p5_ARpatient', 'q90_ARpatient', 'mean_ARpatient',...
            'p1_ARnurse','p5_ARnurse', 'q90_ARnurse', 'mean_ARnurse'} ;
    ...
    for Vol_fac_dbl = Vol_fac_dbl_vals
        ...
        for ACH_p = ACH_p_vals 
            ...
            for ACH_br = ACH_br_vals
                ...
                [q90_AR, p1_AR, p5_AR, avg_AR] = compute_summary_stats(data_table.AR) ;
                [q90_ARpatient, p1_ARpatient, p5_ARpatient, avg_ARpatient] = compute_summary_stats(data_table.AR_patient) ;
                [q90_ARnurse, p1_ARnurse, p5_ARnurse, avg_ARnurse] = compute_summary_stats(data_table.AR_nurse) ;
                ...
                %p10
                summary_output.p5_AR(scenario_index) = p5_AR;
                summary_output.p5_ARpatient(scenario_index) = p5_ARpatient;
                summary_output.p5_ARnurse(scenario_index) = p5_ARnurse;
                ...
                var_label_prefix = ['index' index_case_config, ...
                    '_pS' pShared_label,...
                    '_ACHp' ACH_p_label,...
                    '_ACHb' ACH_br_label,...
...
%% case 1: plot secondary cases in nurses
% compare to nurses

v1 = output_table.indexdifferent_pS0p0_ACHp6p0_ACHb2p0_V1p5_AR_nurse;
v2 = output_table.indexsame_pS1p0_ACHp6p0_ACHb2p0_V1p5_AR_nurse;
v3 = output_table.indexdifferent_pS1p0_ACHp6p0_ACHb2p0_V1p5_AR_nurse;

% legend text. 
leg_txt = {'single-occupancy', 'double-occupancy (same room)', 'double-occupancy (different rooms)'};
...
fid = 1;
nbins = 7;
% histogram_plot_with_quantiles(input_mat, qs, fid, leg_txt, nbins)
histogram_bar_plot(input_mat, qs, fid, leg_txt, nbins)
xticks([1, 2, 3, 4, 5, 6, 7])
xticklabels([0, 1, 2, 3, 4, 5, 6])
title(['secondary cases in nurses'])
xlabel(xlabel_text)
ylabel(['frequency (N = ' N ')'])


%% case 2: plot secondary cases in patients

v1 = output_table.indexdifferent_pS0p0_ACHp6p0_ACHb2p0_V1p5_AR_patient;
v2 = output_table.indexsame_pS1p0_ACHp6p0_ACHb2p0_V1p5_AR_patient;
v3 = output_table.indexdifferent_pS1p0_ACHp6p0_ACHb2p0_V1p5_AR_patient;
...
title(['secondary cases in patients'])
xlabel(xlabel_text)
ylabel(['frequency (N = ' N ')'])
...
%% local functions 
function boxchart_with_mean_and_quantile(input_matrix, q, figure_index)

    mean_vals = mean(input_matrix);
    q_vals = quantile(input_matrix, q);

    figure(figure_index)
    b = boxchart(input_matrix);
    b.BoxWidth = 1;
    b.JitterOutliers = 'on';
    b.MarkerStyle = '.';
    b.MarkerSize = 10;
...
% fiddly. 
function violin_swarm_with_mean_and_quantile_nzv(input_matrix, q, figure_index)
    ...
    violinplot(input_matrix_g0);
    hold on
    swarmchart(categorical(["1", "2", "3"]), input_matrix_g0)
...
function point_cloud_with_mean_and_quantile(input_matrix, x_vals, q, jitter_width, bandwidth, figure_index)
...
function [q90, p1, p5, avg] = compute_summary_stats(input_vec) 

        q90 = quantile(input_vec, 0.9);
        p5 = sum(input_vec > 5) / size(input_vec, 1);
