...
output_label_outer = "full_run_FSvsBeta_14d_sync_masks_2024_12_05"
...
function configure_constants(model_type::String, 
    ...
    #event lists: 
    fname_trajectories_hcw = joinpath(dirname_trajectories, "trajectory_hcw_" * trajectory_label * ".csv")
    fname_trajectories_patient = joinpath(dirname_trajectories, "trajectory_patient_" * trajectory_label * ".csv")
...
function configure_run(seed_infections::Int64, 
                       ...
                       q_vs_t_flag::Bool,
                       n_index_cases::Int64,
                       mask_efficacy::Float64)::TransmissionDynamics.Config
...
function run!(config)
    ...
    if config.index_case_type == "nurse"
        TransmissionDynamics.initialise_infections_hcw!(config, hcws, config.n_index_cases)
        TransmissionDynamics.initialise_infections_patient!(config, patients, 0)
    elseif config.index_case_type == "patient"
        TransmissionDynamics.initialise_infections_patient!(config, patients, config.n_index_cases)
...
function main(fname_tag::AbstractString) 
    ...
    betas = [100.0, 250.0, 500.0, 1000.0, 2500.0, 5000.0, 10000.0, 25000.0, 50000.0, 100000.0]#[0.15]


    index_case_types = ["patient"]# ["nurse", "patient"]
    ...
    for experiment_type in ["OB"]

        for index_case_type in index_case_types 

            p_vals = [0.0, 1.0]
            f_vals = [0.0]
            ACHpr_vals = [6.0]#[6.0, 12.0]
            ACHbr_vals = [2.0]
            VfacDbl_vals = [1.5]#, 2.0]
                            ...
                            for VfacDbl in VfacDbl_vals
                                ...
                                for b in betas 
                                    ...
                                    #for i in 1:n_runs
                                        ...
                                        c = configure_run(seed_i, seed_i, b, ex, q_vs_t_flag, n_index_cases, mask_efficacy)
